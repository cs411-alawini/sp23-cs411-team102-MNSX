...
On a more complex level, <mark>we will have users enter username and password to log in.</mark> With this, we aim to store recent searches for each user locally. To do this, we will have to store recent searches in a new table of our choosing. <mark>For insertion interface, we will allow users to add games into the database; For deletion interface, users can delete their searches from their search history; For updating interface, we will allow users to edit the game info in the existing game entries (like fixing bug info).</mark> The most complex feature we aim to complete is connecting the user with other people who have similar game interests. Lastly, on the most general level, we want to host our web application through googledomains.com.
